...
interface YouTrackApi {
    //tasks
    @GET("/api/issues/?fields=id,summary,idReadable,project(shortName),created,customFields(value(minutes,name),minutes,iconUrl,name,id,projectCustomField(field(value())))")
    ...
    @GET("/api/issues/{id}?fields=id,summary,project(shortName),created,customFields(value(minutes,name),minutes,iconUrl,name,id,projectCustomField(field(value())))")
